...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('should return JWT token for valid credentials without 2FA', async () => {
      ...
      expect(userRepository.findOne).toHaveBeenCalledWith(0, 'john@test.com');
  ...
  describe('verifyOtpLogin', () => {
    it('should return JWT token for valid OTP', async () => {
      ...
      expect(userOtpRepository.markUsed).toHaveBeenCalledWith(
        123,
        expect.objectContaining({ id: 'otp-1' }),
      );
      expect(result).toEqual({
