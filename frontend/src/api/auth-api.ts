...
export const getEmailPreferencesApi = async (): Promise<EmailPreferences> => {
  const response = await api.get("/v1/users/me/email-preferences");
  return response.data;
};
...
export const updateEmailPreferencesApi = async (
  ...
  const response = await api.put("/v1/users/me/email-preferences", prefs);
  return response.data;
};
...
export const verifyOtpLoginApi = async (tempToken: string, otp: string) => {
  const response = await api.post("/v1/auth/verify-otp-login", {
    tempToken,
    otp,
  });
  return response.data;
...
export const toggle2faApi = async (enabled: boolean) => {
  const response = await api.post("/v1/auth/toggle-2fa", { enabled });
  return response.data;
};
